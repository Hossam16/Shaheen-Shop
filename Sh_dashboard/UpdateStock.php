...
if(isset($_POST['submitin']))
 {
    ...
    if(explode(".",$filename)[1]=='xlsx')
    {
                ...
                if(explode(".",$filename)[0]=="StockUpdate")
                {
                        // Upload file
                        if(unlink("StockUpdate.xlsx"))
                        {
                                if(move_uploaded_file($_FILES['file']['tmp_name'][$i],'StockUpdate.xlsx'))
                                {
                                    date_default_timezone_set('Africa/Cairo');
                                    copy("StockUpdate.xlsx", "./bulk/stock/" . date("Y-m-d H-i-s") . ".xlsx"); 
                ...
                {
                $faildmes = $faildmes . $filename . " " ."Not Right Name (StockUpdate.xlsx)" . "<br>";
                }
    }else
    {
        ...
        <div class="col-sm-12">
            <div class="card card-primary">
              <div class="card-header">
                ...
                <br>
                <button name="create_excel" id="btnExport" onclick="exportTableToExcel('downloadsample','StockUpdate');" class="btn btn-success" <?php if($_SESSION['type']=='viewer') {echo'disabled';}?>>Template</button>
                <script> 
        ...
        downloadLink.click();
            ...
            <div class="card card-success">
              ...
              <div class="card-body">
                ...
                if ( $xlsx = SimpleXLSX::parse('StockUpdate.xlsx') ) {
                    echo '<table id="download"><tbody>';
                    $i = 0;
                    foreach ($xlsx->rows() as $elt) {
                        if($i==0)
                        {
                            echo "<tr><th>" . $elt[0] . "</th><th>" . $elt[1] . "</th><th>". "Affected Rows" . "</th>";
                        }else
                        {
                        ...
                        $saleprice = $elt[1];
                        if($i>0) {
                            $sql6655 = "UPDATE products SET products.Stock=$saleprice WHERE products.Photo = '$pid.jpg'";
