...
if (isset($_POST['update'])) {
    ...
    for ($i = 0; $i < $y; $i++) {
        ...
        $insertSizeUpdate = "INSERT INTO `ShipmentLog`(`AID`, `PID`, `From`, `To`, `PriceFrom`, `PriceTo`,`SalePrdiceFrom`, `SalePrdiceTo`, `CreatedDate`) VALUES ($AID,$pid,'$resultSize','$SI','$resultPrice','$price','$resultSalePrice','$sprice','$date')";
        mysqli_query($conn65->datacon(), $insertSizeUpdate);
        $sql6655 = "UPDATE products SET products.Price=$price,products.BSale=$bsale,products.SalePrice=$sprice,products.Stock=$stock,products.SID=$sid,products.Size='$SI',products.Status=$active WHERE products.ID=$pid;";
                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>سعر بعدالخصم</th>
                                            <th>Stock <input type="number" class="form-control" onchange="SelectALLStock(this.value)"></th>
                                            <th>قسم
                                                <select class="form-control" onchange="SelectALLCate(this.value)">
                                                    <option></option>
                                                    <?php
                                                    $conn6545 = new config();
                                                    $sql6555 = "SELECT * FROM nsub";
                                                    $result6555 = $conn6545->datacon()->query($sql6555);
                                                    ?>
                                                    <?php while ($row655 = $result6555->fetch_assoc()) { ?>
                                                        <option value="<?php echo $row655['ID'] ?>"><?php echo $row655['subar'] ?></option>
                                                    <?php } ?>
                                                </select>
                                            </th>
                                            <th>Shipment
                                        ...
                                        ?>
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td><input type="number" class="form-control" id="Pstock<?php echo $gg ?>" name="Pstock<?php echo $gg ?>" value="<?php echo $row65['Stock'] ?>"></td>
                                                    <td>
                                                        <?php
                                                        $conn655 = new config();
                                                        $sid = $row65['SID'];
                                                        ...
                                                        $sql655 = "SELECT * FROM nsub WHERE nsub.ID !=$sid";
                                                        $result655 = $conn655->datacon()->query($sql655);
                                                        ?>
                                                        <div class="form-group">
                                                            <label>قسم فرعي</label>
                                                            <select class="form-control" id="Ps<?php echo $gg ?>" name="Ps<?php echo $gg ?>">
                                                                <?php if ($sid == 0) { ?>
                                                                    <option value="<?php echo $row65['SID'] ?>"><?php echo $sid ?></option>
                                                                <?php  } else {
                                                                    $sql6555 = "SELECT * FROM nsub WHERE nsub.ID=$sid";
                                                                    $result6555 = $conn655->datacon()->query($sql6555);
                                                                    $sabar = $result6555->fetch_assoc()['subar'];
                                                                ?>
                                                                    <option value="<?php echo $row65['SID'] ?>"><?php echo $sabar ?></option>
                                                                <?php } ?>
                                                                <?php while ($row655 = $result655->fetch_assoc()) { ?>
                                                                    <option value="<?php echo $row655['ID'] ?>"><?php echo $row655['subar'] ?></option>
                                                                <?php } ?>
                                                            </select>
                                                        </div>
                                                    ...
                                                    <td>
                                                        <select class="form-control" id="SI<?php echo $gg ?>" name="SI<?php echo $gg ?>">
                                                            <?php
                                                            if ($row65['Size'] == 'Small') { ?>
                                                                <option value="<?php echo $row65['Size'] ?>"><?php echo $row65['Size'] ?></option>
                                                                <option value="Big">Big</option>
                                                                <option value="Free">Free</option>
                                                                <option value="Med60">Med60</option>
                                                                <option value="Med100">Med100</option>
                                                            <?php  } elseif ($row65['Size'] == 'Big') { ?>
                                                                <option value="<?php echo $row65['Size'] ?>"><?php echo $row65['Size'] ?></option>
                                                                <option value="Small">Small</option>
                                                                <option value="Free">Free</option>
                                                                <option value="Med60">Med60</option>
                                                                <option value="Med100">Med100</option>
                                                            <?php  } elseif ($row65['Size'] == 'Med60') { ?>
                                                                <option value="<?php echo $row65['Size'] ?>"><?php echo $row65['Size'] ?></option>
                                                                <option value="Big">Big</option>
                                                                <option value="Small">Small</option>
                                                                <option value="Free">Free</option>
                                                                <option value="Med100">Med100</option>
                                                            <?php  } elseif ($row65['Size'] == 'Med100') { ?>
                                                                <option value="<?php echo $row65['Size'] ?>"><?php echo $row65['Size'] ?></option>
                                                                <option value="Big">Big</option>
                                                                <option value="Small">Small</option>
                                                                <option value="Free">Free</option>
